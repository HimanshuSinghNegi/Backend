...
const autoLogIn = async (req, res) => {
    try {
        ...
        if (token && token != undefined) {
            ...
            sendSuccessMessage(res, 200, 'Logged in by token', null, await createToken(user, process.env.ACCESS_SECRET_KEY, process.env.ACCESS_TOKEN_EXPIRY_TIME));
        } else {
            sendErrorMessage(res, 400, 'please provide token in headers');
        }
    } catch (err) {
...
const createAccessToken = async (req, res) => {
    try {
        ...
        if (token && token !== undefined) {
            ...
            sendSuccessMessage(res, 200, null, await createToken(user, process.env.ACCESS_SECRET_KEY, process.env.ACCESS_TOKEN_EXPIRY_TIME));
        } else {
            sendErrorMessage(res, 400, 'Please provide token in headers');
        }
...
const createToken = async (userData, secretKey, expireIn) => {
    const token = await jwt.sign({ roleId: userData.roleId, id: userData.id }, secretKey, { expiresIn: expireIn });
    return token;
};
...
const userSignIn = async (req, res) => {
    try {
        ...
        sendSuccessMessage(res, 200, 'logged in successfully', null, accessToken, refreshToken);
